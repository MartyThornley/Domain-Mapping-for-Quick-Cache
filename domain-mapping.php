<?php
/*
Plugin Name: Domain Mapping for Quick Cache
Plugin URI: http://blogsitedev.com
...
/*  
	...
	This is based on Domain Mapping (version 3.0.2) by Barry Getty at Incsub ( WPMUDev ):
	http://premium.wpmudev.org/project/domain-mapping
	Author Url was: http://caffeinatedb.com
	Which was Copyright Incsub (http://incsub.com/)
    and Based on an original by Donncha (http://ocaoimh.ie/)
...
/*
 * Fixes for Quick Cache - added by Marty
 */
	add_action( 'init' , 'delete_cache_with_domain' , 1 );
	
	function delete_cache_with_domain() {
		
		global $current_blog, $dm_map;
		
		if ( is_admin() && class_exists( 'c_ws_plugin__qcache_purging_routines' ) && class_exists( 'domain_map' ) ) {
		
			if ( strpos($_SERVER['REQUEST_URI'], 'tools.php?page=domainmapping') ) {
			
				if ( isset( $_POST['action'] ) ) {
					
					// when adding domain, we should delete old cache
					if ( $_POST['action'] == 'add' ) {
					
						c_ws_plugin__qcache_purging_routines::purge_cache_dir ($current_blog);
					
					// when deleting domain, we should delete cache using domain
					} elseif ( $_POST['action'] == 'delete' ) {
					
						$faux_blog = clone $current_blog;
						
						if ( is_object ( $faux_blog ) && $faux_blog->blog_id && $dm_map->domain_mapping_siteurl(false) != '' ) {
							$faux_blog->domain = preg_replace( '/https?:\/\//i', '', $dm_map->domain_mapping_siteurl(false) );
	...
	function dm_clear_routine_hook() {
		global $current_blog;
		$faux_blog = clone $current_blog;
		$domain = new domain_map; 
		
		if ( ( $nonce = $_POST["ws_plugin__qcache_ajax_clear"] ) && wp_verify_nonce ( $nonce, "ws-plugin--qcache-ajax-clear" ) ) {
			if ( class_exists('domain_map') && is_object ( $faux_blog ) && $faux_blog->blog_id && $domain->domain_mapping_siteurl(false) != '' ) {
				$faux_blog->domain = preg_replace( '/https?:\/\//i', '', $domain->domain_mapping_siteurl(false) );
				if ( is_multisite () && !is_main_site () && $faux_blog && $faux_blog->domain && $faux_blog->domain != $current_blog->domain ) {
					c_ws_plugin__qcache_purging_routines::purge_cache_dir ($faux_blog);
				}
			}
...
class domain_map {
	...
	function __construct() {
		...
		if( ( is_admin() || strpos(addslashes($_SERVER["SCRIPT_NAME"]),'/wp-login.php') !== false ) && strpos(addslashes($_SERVER["SCRIPT_NAME"]),'postpass') !== false ) {
	...
	function setup_plugin() {
		...
		if(function_exists('is_supporter') && $sup == '1') {
			...
			add_action( 'manage_sites_custom_column', array(&$this, 'add_domain_mapping_field'), 1, 3 );
			...
			update_option( 'blog_mapped_domain' , $url );
	...
	function handle_domain_page() {
		...
		if ( !empty( $_POST[ 'action' ] ) && !empty( $_POST[ 'domain' ] ) ) {
			
			check_admin_referer( 'domain_mapping' );
			switch( $_POST[ 'action' ] ) {
				case "add":
					$domain = $this->db->escape( preg_replace( "/^www\./", "", $_POST[ 'domain' ] ) );
					echo '<div class="updated">Domain: '.strtolower($domain).' was successfully added. Please see our instructions on domain mapping if it is not working yet.</div>';
				...
				case "delete":
					...
					echo '<div class="updated">Domain: '.strtolower($domain).' was successfully deleted. If there was a problem, please contact the site administrator for help.</div>';
